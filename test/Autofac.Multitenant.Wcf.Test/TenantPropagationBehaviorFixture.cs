...
public class TenantPropagationBehaviorFixture
{
    ...
    public void ApplyDispatchBehavior_Client_NoOp()
    {
        ...
        var exception = Record.Exception(() => behavior.ApplyDispatchBehavior(null!, (EndpointDispatcher)null!));
        Assert.Null(exception);
    }
    ...
    {
        var behavior = new TenantPropagationBehavior<string>(new StubTenantIdentificationStrategy());
        var ex = Assert.Throws<ArgumentNullException>(() => behavior.ApplyDispatchBehavior(null!, (ServiceHostBase)null!));
